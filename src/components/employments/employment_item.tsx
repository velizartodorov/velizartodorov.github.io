...
// Title/place/period go through ItemHeaderRow's own columns, same as every other row in the app,
// instead of a stacked title-then-period line - so they line up horizontally like the entry header.
// The description is part of the always-visible header, not TimelineRow's collapsible children -
// each position's own chevron/collapse is removed, only the entry's own chevron toggles anything.
...
const EmploymentItem: FC<{ item: Employment; index: number }> = ({ item, index }) => {
    ...
    return (
        <TimelineEntry
            id={String(index)}
            icon={{ src: item.icon, alt: `${item.company} logo` }}
            header={
                <ItemHeaderRow
                    title={headerTitle}
                    place={headerPlace}
